'use strict';class PageComponent{constructor(){/**
 ...
 */
function AlertAction(){this.sMessage=null;this.sSize='medium';this.loadProperties=function(aProps){for(var i=0;i<aProps.length;i++){if(aProps[i].Name=='Message'){this.sMessage=aProps[i].Value}else if(aProps[i].Name=='Size'){this.sSize=aProps[i].Value}}};this.execute=function(AjaxCallID){Framework.modules.Dialog.alert(this.sMessage,function(){Framework.modules.Ajax.processNextAction(AjaxCallID)},this.sSize)}};
/**
 *
 *
 * LICENSE: This source file is subject to the LogikBuild license
 ...
 * @license src/Framework/LICENSE.md
 */
function JQueryAction(){this.sCode=null;this.loadProperties=function(aProps){for(var i=0;i<aProps.length;i++){if(aProps[i].Name=='Code'){this.sCode=aProps[i].Value}}};this.execute=function(AjaxCallID){if(this.sCode!=null){eval(decodeURIComponent(atob(this.sCode)))}
