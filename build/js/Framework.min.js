 ...
 * 	@license 	LICENSE.md
 */
Framework.modules.Dialog={_id:0,_activeWindowFields:[],/**
 ...
 * 	@license 	LICENSE.md
 */
Framework.modules.Utilities={_uniqidSeed:null,initialize:function(){console.log('Init utilities');Number.prototype.round=function(places){return+(Math.round(this+"e+"+places)+"e-"+places)}},isEmpty:function(v){return(v===undefined||v==null||v=='')?!0:!1},parseFloat:function(v){if(v==''){v='0'}
return parseFloat(v)},uniqid:function(prefix,moreEntropy){var retId;var reqWidth=8;if(typeof prefix==='undefined'){prefix=''}
if(this._uniqidSeed==null){this._uniqidSeed=Math.floor(Math.random()*0x75bcd15)}
this._uniqidSeed++;retId=prefix;retId+=this._formatSeed(parseInt(new Date().getTime()/1000,10),reqWidth);retId+=this._formatSeed(this._uniqidSeed,reqWidth);if(moreEntropy){retId+=(Math.random()*10).toFixed(8).toString()}
return retId},_formatSeed:function(seed,reqWidth){seed=parseInt(seed,10).toString(16);if(reqWidth<seed.length){seed=seed.slice(seed.length-reqWidth)}
if(reqWidth>seed.length){seed=Array(1+(reqWidth-seed.length)).join('0')+seed}
return seed},};
/**
 *
 *	LICENSE: This source file is subject to the LogikSuite Framework license
 ...
 * 	@license 	LICENSE.md
 */
Framework.modules.UI={_lastEvent:null,_delayTimer:0,initialize:function(){jQuery(window).on('keydown',function(e){Framework.modules.UI._lastEvent=e});jQuery(window).on('keyup',function(e){Framework.modules.UI._lastEvent=e})},delay:function(callback,ms,data){if(Framework.modules.UI._delayTimer!=0){clearTimeout(Framework.modules.UI._delayTimer)}
Framework.modules.UI._delayTimer=0;Framework.modules.UI._delayTimer=setTimeout(callback,ms,data)},};jQuery(document).ready(function(){Framework.initialize()})